...
class JeenyConfigurations: NSObject {
    
    class func getCloudSettings () -> [String: Any] {
        
        if let kCloudSettings = Bundle.main.path(forResource: kConfigurations, ofType: kPlist) {
            
            if let dic = NSDictionary(contentsOfFile: kCloudSettings) as? [String: Any] {
                let configurationName = Bundle.main.configuration
    ...
    class func getBuildEnvironmentSettings () -> [String: Any] {
        ...
        let buildEnvironment = Bundle.main.buildEnvironment
